...
class Generator(nn.Module):
    ...
    def first_block(self):
        ...
        if self.flag_norm_latent:
            layers.append(PixelwiseNormLayer())
        layers = deconv(layers, self.nz, ndim, 4, 1, 3,
                        self.flag_leaky, self.flag_wn, self.flag_pixelwise)
        layers = deconv(layers, ndim, ndim, 3, 1, 1,
                        self.flag_leaky, self.flag_wn, self.flag_pixelwise)
    ...
    def intermediate_block(self, resl):
        ...
        if halving:
            layers = deconv(layers, ndim * 2, ndim, 3, 1, 1,
                            self.flag_leaky, self.flag_wn, self.flag_pixelwise)
            layers = deconv(layers, ndim, ndim, 3, 1, 1,
                            self.flag_leaky, self.flag_wn, self.flag_pixelwise)
        else:
    ...
    def to_rgb_block(self, c_in):
        layers = []
        layers = deconv(layers, c_in, self.nc, 1, 1, 0,
                        self.flag_leaky, self.flag_wn, self.flag_pixelwise, only=True)
        if self.flag_tanh:
    ...
    def grow_network(self, resl):
        ...
        if resl >= 3 and resl <= 9:
            print('Growing Generator network [{}x{} to {}x{}]... Just a second :)'.format(
                int(pow(2, resl - 1)), int(pow(2, resl - 1)), int(pow(2, resl)), int(pow(2, resl))))
